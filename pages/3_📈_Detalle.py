...
with st.expander("Filters", expanded=True):
    ...
    with c1:
        site_filter = st.selectbox(
            "**Select site:**",
            options=read_machines()["site_name"].unique(),
            # default=["Antapaccay"],
            key="site_options",
        )
    ...
    with c2:
        model_filter = st.selectbox(
            "**Select model:**",
            options=read_machines()["model"].unique(),
            # default=["980E-5"],
            key="model_options",
        )
        machine_filter = st.selectbox(
            "**Select machine:**",
            options=df_breakdowns["equipment"].unique(),
            # default=["980E-5"],
            key="machine_options",
        )

df = df.filter((pl.col("equipment") == machine_filter))
